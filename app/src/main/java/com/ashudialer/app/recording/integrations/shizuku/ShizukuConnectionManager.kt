...
package com.ashudialer.app.recording.integrations.shizuku
...
import com.ashudialer.app.recording.services.ShellService
...
class ShizukuConnectionManager(
    ...
    companion object {
        ...
        /**
         * This permission code is used to identify the permission request when the user responds to the dialog. It can be any unique integer.
         */
        ...
        fun hasPermission(context: Context? = null): Boolean {
            return try {
                if (isAvailable()) {
                    ...
                    if (context == null) {
                        Log.v(TAG, "Cannot check permission via Shizuku API. Context is null, will not perform potentially out-of-sync Android permission check fallback. High probability of being the intended behavior, but might not.")
                        return false
                    }
                ...
                if (isAvailable()) {
                    ...
                    when (result) {
                        PackageManager.PERMISSION_GRANTED -> {
                            Log.d(TAG, "Remote Shizuku server has permission: $permissionName")
                            return true
                        }
                        PackageManager.PERMISSION_DENIED -> {
                            Log.w(TAG, "Remote Shizuku server does NOT have permission: $permissionName")
                            return false
                        }
                        else -> Log.v(TAG, "Unexpected result from checkRemotePermission: $result for permission: $permissionName")
                    }
                    return false
                } else {
                    Log.v(TAG, "Cannot check remote Shizuku server available permission. Server is not available.")
                    return false
                }
            } catch (e: Exception) {
                Log.e(TAG, "Error while checking remote Shizuku server permission", e)
                false
            }
        }
        ...
        fun startServer(context: Context, authKey: String) {
            try {
                if (isAvailable()) {
                    Log.i(TAG, "Shizuku server is already running, no need to send start broadcast")
                    return
                }
                ...
                Log.i(TAG, "Sent broadcast to start Shizuku server to $packageName")
            } catch (e: Exception) {
                Log.e(TAG, "Failed to send broadcast to start Shizuku server", e)
            }
        }
        ...
        fun stopServer(context: Context, authKey: String) {
            try {
                if (!isAvailable()) {
                    Log.i(TAG, "Shizuku server is already stopped, no need to send stop broadcast")
                    return
                }
                ...
                context.sendBroadcast(intent)
                Log.i(TAG, "Sent broadcast to stop Shizuku server to $packageName")
            } catch (e: Exception) {
                Log.e(TAG, "Failed to send broadcast to stop Shizuku server", e)
            }
        }
        ...
        suspend fun waitForServer(timeoutMillis: Long = 30000, pollIntervalMillis: Long = 250): Boolean {
            ...
            Log.w(TAG, "Timed out waiting for Shizuku server after ${timeoutMillis}ms")
            return false
        }
    }
    ...
    suspend fun getShellService(): IShellService = suspendCancellableCoroutine { continuation ->
        ...
        val connection = object : ServiceConnection {
            ...
            override fun onServiceDisconnected(name: ComponentName?) {
                Log.d(TAG, "ShellService disconnected prematurely")
        ...
        fun bindServiceInternal() {
            try {
                Log.i(TAG, "Binding ShellService...")
            ...
            override fun onRequestPermissionResult(requestCode: Int, grantResult: Int) {
                if (requestCode == PERMISSION_REQUEST_CODE) {
                    ...
                    if (grantResult == PackageManager.PERMISSION_GRANTED) {
                        Log.d(TAG, "Permission granted, proceeding to bind")
                        bindServiceInternal()
                    } else {
                        Log.w(TAG, "Shizuku permission denied, cannot continue with binding")
                        if (continuation.isActive) {
        ...
        if (hasPermission()) {
            ...
            Log.w(TAG, "Cannot bind yet, missing permission, requesting Shizuku permission...")
        ...
        if (serviceConn != null) {
            try {
                ...
                if (isAvailable()) {
                    ...
                    Log.i(TAG, "ShellService was unbound")
                } else {
                    Log.d(TAG, "ShellService binder is not available for a unbind. It has already been killed by the Shizuku Server. No issues here.")
                }
            } catch (e: Exception) {
                Log.e(TAG, "Unexpected error occurred while trying to unbind ShellService", e)
            }
        }
