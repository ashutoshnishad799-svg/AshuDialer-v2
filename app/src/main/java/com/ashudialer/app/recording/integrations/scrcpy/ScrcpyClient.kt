...
class ScrcpyClient(
    ...
    fun start() {
        ...
        try {
            ...
            Log.d(TAG, "Codec FourCC: received=0x${receivedFourCC.toString(16)} resolved=${resolvedCodec.cliKey} expected=${expectedCodec.cliKey}")

            // Warn if the server is encoding with a different codec than we requested.
            if (resolvedCodec != expectedCodec) {
                Log.w(TAG, "Codec mismatch: requested ${expectedCodec.cliKey} but server sent ${resolvedCodec.cliKey} (FourCC=0x${receivedFourCC.toString(16)})")
            }
            ...
            while (running) {
                ...
                if (!header.isMedia) {
                    Log.w(TAG, "Unexpected session packet detected on audio stream! flags=0x${header.rawPtsAndFlags.toString(16)}")
                }
                ...
                //Log.v(TAG, "Packet: pts=${header.pts} config=${header.isConfig} keyFrame=${header.isKeyFrame} media=${header.isMedia} size=${header.payloadSize}")
            ...
            Log.d(TAG, "Given stream was fully read (EOF), nothing else to parse, ending normally.")
