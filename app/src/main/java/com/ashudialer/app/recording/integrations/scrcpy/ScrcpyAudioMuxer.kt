...
class ScrcpyAudioMuxer(
    ...
    fun initialize(codec: ScrcpyAudioCodec) {
        ...
        Log.d(TAG, "Initialising muxer: codec=${codec.cliKey} format=${codec.outputFormat} path='$outputDisplayPath'")
    ...
    fun writePacket(packet: ScrcpyClient.AudioPacket, codec: ScrcpyAudioCodec) {
        ...
        if (!isMuxerStarted || audioTrackIndex < 0) {
            Log.w(TAG, "writePacket(): muxer not ready – dropping frame")
            return
        }
        ...
        if (firstPacketTimeNanos == -1L) {
            ...
            Log.d(TAG, "First audio frame: wall-clock origin set, pts=0")
        } else {
            ...
            if (gapNanos > GAP_THRESHOLD_NANOS) {
                ...
                Log.d(TAG, "Detected huge gap silence/pause of ${gapNanos / 1_000_000} ms. Squashing ${ignoredNanos / 1_000_000} ms.")
            }
            lastPacketWallClockNanos = nowNanos
        }
    ...
    override fun close() {
        if (isMuxerStarted) {
            Log.d(TAG, "Finalising muxer for '$outputDisplayPath'")
            runCatching { muxer?.stop() }.onFailure { e ->
                Log.e(TAG, "Muxer stop failed (file may be incomplete): ${e.message}")
            }
        }
    ...
    private fun addAudioTrack(configData: ByteArray, codec: ScrcpyAudioCodec) {
        ...
        if (csdBytes == null) {
            Log.e(TAG, "Empty config data received. Cannot initialize audio track.")
            return
        }
        ...
        if (audioTrackIndex < 0) {
            Log.e(TAG, "Failed to add audio track (addTrack returned $audioTrackIndex)")
            return
        }

        muxer?.start()
        isMuxerStarted = audioTrackIndex >= 0
        Log.d(TAG, "Audio track added (index=$audioTrackIndex mime=${codec.mimeType}) – muxer started")
    }
}
