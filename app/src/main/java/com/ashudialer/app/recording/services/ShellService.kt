...
package com.ashudialer.app.recording.services
...
class ShellService : IShellService.Stub {
    ...
    override fun startRecording(
        ...
        if (isRecordingActive.get()) {
            Log.w(TAG,"startRecording() rejected: a session is already active")
            return null
        }

        try {
            Log.i(TAG,"Initialising the ShellService recording pipeline...")
            ...
            if (!serverJarFile.exists() || !ServerExtractor.verifyServerHash(serverJarFile)) {
                Log.w(TAG,"Server JAR absent or SHA-256 mismatch at $serverPath - aborting")
                return null
            }
            ...
            Log.d(TAG,"Listening on abstract socket '$serverFullSocketName'")
            ...
            Log.i(TAG,"scrcpy-server launched successfully")

            // 5. Start Helper coroutines
            spawnLogConsumerCoroutine(scrcpyProcess!!)
            spawnProcessMonitorCoroutine(scrcpyProcess!!)

            Log.i(TAG,"Recording pipeline established. Returning pipe read-end to app process.")
            return pipeReadEnd // ← returned to RecordingForegroundService via Binder
        } catch (e: Exception) {
            Log.e(TAG,"Critical failure during pipeline startup: ${e.message}", e)
    ...
    override fun stopRecording() {
        ...
        if (!isRecordingActive.compareAndSet(true, false)) {
            Log.w(TAG, "stopRecording() called but no active session - skipping redundant cleanup")
            return
        }

        Log.i(TAG,"Stopping scrcpy-server process...")
        ...
        try {
            ...
            Log.w(TAG, "Interrupted while waiting for scrcpy-server exit: ${e.message}")
        }

        // Wait for the relay job to copy remaining socket data to the pipe for downstream app (Time-bounded).
        Log.d(TAG,"Waiting for relay coroutine to finish copying late bytes...")
        runCatching {
        ...
        Log.d(TAG,"Cancelling all jobs running from the shell coroutine scope...")
        ...
        Log.d(TAG,"Ensuring client socket connection is closed...")
        ...
        Log.d(TAG,"Closing audio pipe write-end...")
        ...
        Log.i(TAG,"Recording pipeline stopped and all ShellService resources were released")
    }
    ...
    override fun grantAppOpByPackage(packageName: String, opName: String, userProfileId: Int): Boolean {
        return try {
            Log.i(TAG, "Executing: appops set --user $userProfileId $packageName $opName allow")
            ...
            if (exitCode == 0 && errorOutput.isEmpty()) {
                Log.i(TAG, "Successfully granted AppOp $opName to $packageName")
                true
            } else {
                Log.e(TAG, "Failed to grant AppOp $opName. Exit code $exitCode. Error: ${errorOutput.ifBlank { "None" }}")
                false
            }
        } catch (e: Exception) {
            Log.e(TAG, "Exception granting AppOp $opName to $packageName", e)
            false
        }
    }
    ...
    override fun grantRole(packageName: String, roleName: String, userProfileId: Int): Boolean {
        return try {
            Log.i(TAG, "Executing: cmd role add-role-holder --user $userProfileId $roleName $packageName")
            val process = ProcessBuilder("cmd", "role", "add-role-holder", "--user", userProfileId.toString(), roleName, packageName).start()
            val exitCode = process.waitFor()
            val errorOutput = process.errorStream.bufferedReader().readText().trim()
            if (exitCode == 0 && errorOutput.isEmpty()) {
                Log.i(TAG, "Successfully granted role $roleName to $packageName")
                true
            } else {
                Log.e(TAG, "Failed to grant role $roleName. Exit code $exitCode. Error: ${errorOutput.ifBlank { "None" }}")
                false
            }
        } catch (e: Exception) {
            Log.e(TAG, "Exception granting role $roleName to $packageName", e)
            false
        }
    }
    ...
    override fun destroy() {
        Log.i(TAG,"ShellService.destroy() – terminating shell process")
        stopRecording()
        exitProcess(0)
    }
    ...
    private fun spawnAudioRelayCoroutine(verbose: Boolean) {
        audioPipeRelayJob = shellScope?.launch(Dispatchers.IO) {
            try {
                ...
                Log.d(TAG,"AudioRelayCoroutine: waiting for scrcpy-server connection...")
                val connection = serverSocket?.accept() ?: run {
                    Log.w(TAG, "AudioRelayCoroutine: server socket was null or closed")
                    return@launch
                }
                clientConnection = connection
                Log.i(TAG,"AudioRelayCoroutine: scrcpy-server connected to our socket server")
                ...
                while (isActive) {
                    ...
                    if (bytesRead == -1) {
                        Log.d(TAG,"AudioRelayCoroutine: socket EOF - scrcpy-server disconnected")
                        break
                    }
                    ...
                    if (verbose && bytesRead > 0) {
                        ...
                        if (now - lastLogTimeMs >= 1000) {
                            lastLogTimeMs = now
                            Log.v(TAG, "AudioRelayCoroutine: relayed $bytesRead bytes. (Wrote to pipe).")
                        }
                    }
                }
            } catch (e: IOException) {
                ...
                if (isRecordingActive.get()) {
                    Log.e(TAG,"AudioRelayCoroutine: unexpected I/O error: ${e.message}", e)
                } else {
                    Log.d(TAG,"AudioRelayCoroutine: I/O error during shutdown (expected): ${e.message}")
                }
            } finally {
                // If we exit the relay (e.g. server crash), trigger a full stop so the app-side
                // muxer finalises the file.
                Log.d(TAG,"AudioRelayCoroutine finished")
                stopRecording()
            }
        }
    }
            ...
            try {
                process.inputStream.bufferedReader().use { reader ->
                    ...
                    while (isActive && line != null) {
                        Log.i(TAG,"[scrcpy-server] $line")
                        line = reader.readLine()
                    }
                }
            } catch (_: InterruptedIOException) {
                // Normal shutdown path: the scope was cancelled and the stream was closed.
                Log.d(TAG,"LogConsumerCoroutine: interrupted (expected during shutdown)")
            } catch (e: IOException) {
                Log.e(TAG,"LogConsumerCoroutine: I/O error: ${e.message}", e)
            } finally {
                Log.d(TAG,"LogConsumerCoroutine finished")
            }
        }
    }
        ...
        shellScope?.launch(Dispatchers.IO) {
            try {
                ...
                if (exitCode != 0 && isRecordingActive.get()) {
                    Log.e(TAG,"ProcessMonitorCoroutine: scrcpy-server crashed (exit code $exitCode)")
                    stopRecording() // Trigger cleanup and file finalisation.
                } else {
                    Log.i(TAG,"ProcessMonitorCoroutine: scrcpy-server exited normally (code $exitCode)")
                }
            } catch (_: InterruptedException) {
                // Normal: the scope was cancelled (service stopped) before the process exited.
                Log.d(TAG,"ProcessMonitorCoroutine: interrupted (expected during shutdown)")
            } finally {
                Log.d(TAG,"ProcessMonitorCoroutine finished")
            }
        }
    }
}