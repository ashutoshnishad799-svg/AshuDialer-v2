...
import com.ashudialer.app.appcalls.ShellService
...
class ShizukuConnectionManager(
    ...
    companion object {
        ...
        /**
         * This permission code is used to identify the permission request when the user responds to the diaAppCallsLogger. It can be any unique integer.
         */
        ...
        fun hasPermission(context: Context? = null): Boolean {
            return try {
                if (isAvailable()) {
                    ...
                    if (context == null) {
                        AppCallsLogger.v(TAG, "Cannot check permission via Shizuku API. Context is null, will not perform potentially out-of-sync Android permission check fallback. High probability of being the intended behavior, but might not.")
                        return false
                    }
                ...
                if (isAvailable()) {
                    ...
                    when (result) {
                        PackageManager.PERMISSION_GRANTED -> {
                            AppCallsLogger.d(TAG, "Remote Shizuku server has permission: $permissionName")
                            return true
                        }
                        PackageManager.PERMISSION_DENIED -> {
                            AppCallsLogger.w(TAG, "Remote Shizuku server does NOT have permission: $permissionName")
                            return false
                        }
                        else -> AppCallsLogger.v(TAG, "Unexpected result from checkRemotePermission: $result for permission: $permissionName")
                    }
                    return false
                } else {
                    AppCallsLogger.v(TAG, "Cannot check remote Shizuku server available permission. Server is not available.")
                    return false
                }
            } catch (e: Exception) {
        ...
        fun startServer(context: Context, authKey: String) {
            try {
                if (isAvailable()) {
                    AppCallsLogger.i(TAG, "Shizuku server is already running, no need to send start broadcast")
                    return
                }
                ...
                AppCallsLogger.i(TAG, "Sent broadcast to start Shizuku server to $packageName")
            } catch (e: Exception) {
                AppCallsLogger.e(TAG, "Failed to send broadcast to start Shizuku server", e)
            }
        }
        ...
        fun stopServer(context: Context, authKey: String) {
            try {
                if (!isAvailable()) {
                    AppCallsLogger.i(TAG, "Shizuku server is already stopped, no need to send stop broadcast")
                    return
                }
                ...
                context.sendBroadcast(intent)
                AppCallsLogger.i(TAG, "Sent broadcast to stop Shizuku server to $packageName")
            } catch (e: Exception) {
                AppCallsLogger.e(TAG, "Failed to send broadcast to stop Shizuku server", e)
            }
        }
        ...
        if (serviceConn != null) {
            try {
                ...
                if (isAvailable()) {
                    ...
                    AppCallsLogger.d(TAG, "ShellService binder is not available for a unbind. It has already been killed by the Shizuku Server. No issues here.")
                }
            } catch (e: Exception) {
                AppCallsLogger.e(TAG, "Unexpected error occurred while trying to unbind ShellService", e)
            }
        }
