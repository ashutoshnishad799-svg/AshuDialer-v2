    ...
    fun start() {
        ...
        try {
            ...
            if (resolvedCodec != expectedCodec) {
                AppCallsLogger.w(TAG, "Codec mismatch: requested ${expectedCodec.cliKey} but server sent ${resolvedCodec.cliKey} (FourCC=0x${receivedFourCC.toString(16)})")
            }
            ...
            while (running) {
                ...
                if (!header.isMedia) {
                    AppCallsLogger.w(TAG, "Unexpected session packet detected on audio stream! flags=0x${header.rawPtsAndFlags.toString(16)}")
                }
                ...
                AppCallsLogger.v(TAG, "Packet: pts=${header.pts} config=${header.isConfig} keyFrame=${header.isKeyFrame} media=${header.isMedia} size=${header.payloadSize}")
            ...
            AppCallsLogger.d(TAG, "Given stream was fully read (EOF), nothing else to parse, ending normally.")
            listener.onStreamEnd(null)
        } catch (e: Exception) {
            // Any other exception indicates an abnormal termination.
            AppCallsLogger.w(TAG, "Stream ended with error: ${e.message}")
